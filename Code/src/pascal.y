...
routine         : routine_head  routine_body { 
                ...
                | routine_head  error routine_body { 
                    $$ = new ast::Routine($1, $3); 
                    yyerrok;
                }
;
...
const_expr_list : const_expr_list  NAME  EQUAL  const_value  SEMI { 
                ...
                | const_expr_list  NAME  EQUAL  const_value  error SEMI{ 
                    $$ = $1;
                    $1->push_back(new ast::ConstDecl(new ast::Name($2), $4));
                    yyerrok;
                }
                ...
                |  NAME  EQUAL  const_value  error SEMI{
                    $$ = new ast::ConstDeclList();
                    $$->push_back(new ast::ConstDecl(new ast::Name($1), $3));
                    yyerrok;
                }
...
type_definition : NAME  EQUAL  type_decl  SEMI { 
                ...
                | NAME  EQUAL  type_decl  error SEMI{ 
                    $$ = new ast::TypeDecl(new ast::Name($1), $3); 
                    yyerrok;
                }
;
...
record_type_decl: RECORD  field_decl_list  END { 
                ...
                | RECORD  field_decl_list  error END{ 
                    $$ = new ast::RecordType($2); 
                    yyerrok;
                }
;
...
field_decl      : name_list  COLON  type_decl  SEMI {
                ...
                | name_list  COLON  type_decl  error SEMI{
                    $$ = new ast::VarDeclList(); 
                    for(auto name: *($1)) 
                        $$->push_back(new ast::VarDecl(name, $3));
                    yyerrok;
...
function_decl   : function_head  SEMI  sub_routine  SEMI {
                ...
                | function_head  SEMI  sub_routine  error SEMI {
                    $$ = new ast::Program($1, $3);
                    yyerrok;
                }
;
...
procedure_decl  :  procedure_head  SEMI  sub_routine  SEMI {
                ...
                | procedure_head  SEMI  sub_routine  error SEMI {
                    $$ = new ast::Program($1, $3);
                    yyerrok;
                }
;
...
if_stmt         : IF  expression  THEN  stmt  ELSE stmt {
                ...
                | IF  expression  error THEN  stmt  ELSE stmt {
                    $$ = new ast::IfStmt($2, $5, $7);
                    yyerrok;
                }
                | IF  expression  THEN  stmt {
                    $$ = new ast::IfStmt($2, $4);
                }
                | IF  expression  error THEN  stmt {
                    $$ = new ast::IfStmt($2, $5);
                    yyerrok;
                }
;
...
repeat_stmt     : REPEAT  stmt_list  UNTIL  expression {
                ...
                | REPEAT  stmt_list  error UNTIL  expression {
                    $$ = new ast::RepeatStmt($5, $2);
                    yyerrok;
                }
;
...
while_stmt      : WHILE  expression DO stmt {
                ...
                | WHILE  expression error DO stmt {
                    $$ = new ast::WhileStmt($2, $5);
                    yyerrok;
                }
;
for_stmt        : FOR  ID  ASSIGN  expression  direction  expression  DO stmt {
                ...
                | FOR  ID  ASSIGN  expression  direction  expression  error DO stmt {
                    $$ = new ast::ForStmt(new ast::Identifier($2), $4, $5, $6, $9);
                    yyerrok;
                }
;
direction       : TO {
...
case_stmt       : CASE expression OF case_expr_list  END {
                ...
                | CASE expression OF case_expr_list  error END {
                    $$ = new ast::CaseStmt($2, $4);
                    yyerrok;
                }
;
...
case_expr       : const_value  COLON  stmt  SEMI {
                ...
                | const_value  COLON  stmt  error SEMI {
                    $$ = new ast::CaseExpr($1, $3);
                    yyerrok;
                }
                |  ID  COLON  stmt  SEMI {
